...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("MonsterChain-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MonsterChainTEST2\r");
            Send(hSocket, "WHO #MonsterChainTEST2\r");
        } else {
            // randomly join #MonsterChain00-#MonsterChain05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MonsterChain%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MonsterChain%02d\r", channel_number).c_str());
        }
